...
int main() {
    ...
    while (!exit) {
        ...
        if(menu_game){
            ...
            if(switch_count % 2 == 0){
                widgets[widgets.size()-2]->set_pos({100,700});
                widgets[widgets.size()-1]->set_pos({840,700});
                dynamic_cast<Tank*>(widgets[widgets.size()-2])->reset(45, 5);
                dynamic_cast<Tank*>(widgets[widgets.size()-1])->reset(135, 5);
            }
